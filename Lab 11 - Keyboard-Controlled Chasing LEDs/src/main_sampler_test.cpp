...
int key_delay(Ps2Core *ps2_p, SsegCore *sseg_p, int &delay)
{
    ...
    // Display initial delay on the 7-segment display
    sseg_p->set_dp(0x80);
    sseg_p->write_1ptn(12, 4);
    sseg_p->write_1ptn(sseg_p->h2s(5), 5);
    sseg_p->write_1ptn(0xB7, 3);

    // Pause detection
    if (ps2_p->get_kb_ch(&ch)) {
        ch_id = (uint8_t)ch;
        if (ch_id == 112 || ch_id == 80) {
            pause_detect = 1;
            sseg_p->write_1ptn(12, 7);
            uart.disp("\n\rPAUSE");
        } else {
            pause_detect = 0;
            sseg_p->write_1ptn(0xFF, 7);
        }
        while (pause_detect == 1) {
            if (ps2_p->get_kb_ch(&ch)) {
                ch_id = (uint8_t)ch;
                if (ch_id == 112 || ch_id == 80) {
                    uart.disp("\n\rRESUME");
        ...
        // Check for keyboard input to update delay digits
        if (ch_id == 0xf0)
        {
            uart.disp("setting digits");
            uart.disp("\n\r");
            int f1_ctr = 0;
            while (f1_ctr < 3)
            {`
                ...
                {
                    ch_id = (uint8_t)ch;
                    if (ch_id >= 48 && ch_id <= 57)
                    {
                        digit[f1_ctr] = ch_id - 48;

                        uart.disp("current digits ");
                        uart.disp(digit[f1_ctr]);
                        uart.disp("\n\r");

                        // Update 7-segment display with new delay digits
                        sseg_p->write_1ptn(sseg_p->h2s(digit[2]), 0);
                        sseg_p->write_1ptn(sseg_p->h2s(digit[1]), 1);
                        sseg_p->write_1ptn(sseg_p->h2s(digit[0]), 2);
    ...
    // Calculate updated delay
    delay = (digit[0] * 100) + (digit[1] * 10) + digit[2];
    uart.disp("delay");
    uart.disp(delay);
    uart.disp("\n\r");
...
void led_chaser_key(Ps2Core *ps2_p, SsegCore *sseg_p, GpoCore *led_p, int &delay)
{

	 for (int i = 0; i < 16; i++)
	 	 	 {
	             led_p->write(1, i);
	             delay = key_delay(ps2_p, sseg_p, delay);
	             sleep_ms(delay);
	             led_p->write(0, i);
	             sleep_ms(delay);
	          }
...
int main() {
   int delay = 100;
   sseg.write_1ptn(sseg.h2s(0), 0);
   sseg.write_1ptn(sseg.h2s(0), 1);
   sseg.write_1ptn(sseg.h2s(1), 2);
   while (1) {

	   led_chaser_key(&ps2,&sseg,&led, delay);
   } //while
} //main
