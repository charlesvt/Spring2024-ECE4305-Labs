...
double get_adc(XadcCore *adc_p, int adc_select){
	double adc_read;
...
void disp_volt(SsegCore *sseg_p, double volt) {
	int volt_norm, volt_norm_ones, volt_norm_tens, volt_norm_huns;
	uint8_t pat_ones, pat_tens, pat_huns;
	volt_norm = (int)(volt*100);
	volt_norm_ones = volt_norm % 10;
	volt_norm_tens = (volt_norm/10) % 10;
	volt_norm_huns = volt_norm/100;
	pat_ones = sseg_p->h2s(volt_norm_ones);
	pat_tens = sseg_p->h2s(volt_norm_tens);
	pat_huns = sseg_p->h2s(volt_norm_huns);
	sseg_p->write_1ptn(pat_ones, 0);
	sseg_p->write_1ptn(pat_tens, 1);
	sseg_p->write_1ptn(pat_huns, 2);
	sseg_p->set_dp(0x04);
}

void uart_volt(int volt) {
   uart.disp("current volt: ");
   uart.disp(volt);
   uart.disp(" V");
   uart.disp("\n\r");
}
...
int main() {
   double volt_in, volt_old;
   timer_check(&led);
   while (1) {
	   volt_in = get_adc(&adc, 0);
	   volt_old = volt_in;

	   disp_volt(&sseg, volt_in);
	   pwm_3color_led_check(&pwm);
	   //spectrum_set(&pwm, volt_in);
